...
class GenericDataClass:
    ...
    def __init__(self, name=None, ignore_props=tuple(), **kwargs):
        self.name = name
        self.ignore_props = ("name", "ignore_props") + ignore_props
        
        for key, value in kwargs.items():
            setattr(self, key, value)
    
    def __iter__(self):
        return iter([v for k, v in vars(self).items() if k not in self.ignore_props])
    
    def __repr__(self):
        attributes = ', '.join(f'{key}={value}' for key, value in vars(self).items() if key not in self.ignore_props)
...
class Management:
    ...
    # prev_event = 0
    # """The type of the previous event for formatting the terminal output.
    #     `False`: No event
    #     `True` : Hotkey event
    # """
    ...
    # @staticmethod
    # def TogglePrevEvent():
    #     Management.prev_event ^= 1
    #     return Management.prev_event
...
class WindowHouse:
    ...
    def GetClassNameHandle(className) -> int | None:
        """Returns the window handle for the given class name."""
        
        return WindowHouse.classNames.get(className, None)
    
    @staticmethod
    def SetClassNameHandle(className: str, value: int):
        """Sets the window handle for the given class name."""
        
        WindowHouse.classNames[className] = value
    
    @staticmethod
    def RememberActiveProcessTitle(fg_hwnd: int=0):
        ...
        explorerAddress = win32gui.GetWindowText(fg_hwnd or win32gui.GetForegroundWindow())
...
class ControllerHouse:
    ...
    def UpdateModifiers_KeyDown(event):
    ...
    def UpdateModifiers_KeyUp(event):
...
# @static_class
# class ShellExplorerWrapper:
#     """Thread-safe wrapper class for accessing an Automation object in a multithreaded environment."""
#     _queue = queue.Queue(1)
#     _shell = Dispatch("Shell.Application")
...
class PThread(threading.Thread):
    ...
    msgQueue: queue.Queue[bool] = queue.Queue()
