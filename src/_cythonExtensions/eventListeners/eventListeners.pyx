...
from cythonExtensions import explorerHelper as expHelper
from cythonExtensions import systemHelper   as sysHelper
from cythonExtensions import windowHelper   as winHelper
from cythonExtensions import keyboardHelper as kbHelper
from cythonExtensions.common import ControllerHouse as ctrlHouse, WindowHouse as winHouse, PThread, Management as mgmt, KB_Con as kbcon
...
cpdef bint HotkeyPressEvent(event):
    ...
    cdef bint suppress_key = mgmt.suppress_all_keys # and not (ctrlHouse.modifiers & ctrlHouse.SHIFT) and not (ctrlHouse.modifiers & ctrlHouse.FN) and
    ...
    #+ Starting the image processing script: '`' + '\' + {Shift}
        ...
        if ctrlHouse.modifiers & ctrlHouse.SHIFT: # Invert without showing the Image window.
            PThread(target=subprocess.call, args=[("python", "-c", "from cythonExtensions import imageInverter as img_inv; img_inv.BeginImageProcessing(False, False)")]).start()
            ...
            if not winHelper.GetHandleByTitle("Image Window"):
                ...
                PThread(target=subprocess.call, args=[("python", "-c", "from cythonExtensions import imageInverter as img_inv; img_inv.BeginImageProcessing(True, False)")]).start()
